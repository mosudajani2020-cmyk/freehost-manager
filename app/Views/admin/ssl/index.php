...
<div class="container-fluid py-4">
  <h4>SSL Certificates <small class="text-muted"><?= (int) $total ?> total</small></h4>
  ...
  <div class="card">
    <div class="table-responsive">
      <table class="table table-sm mb-0">
        <thead><tr><th>ID</th><th>Hostname</th><th>Hosting</th><th>Status</th><th>Provider</th><th>Expires</th><th></th></tr></thead>
        <tbody>
          <?php foreach ($rows as $r): ?>
            <tr>
              ...
              <td><span class="badge bg-<?= $r['status']==='active'?'success':($r['status']==='failed'?'danger':'warning') ?>"><?= e($r['status']) ?></span></td>
              <td><?= e($r['provider']) ?></td>
              <td class="small"><?= e($r['expires_at'] ?? '-') ?></td>
              <td><a href="/admin/ssl/<?= (int) $r['id'] ?>" class="btn btn-sm btn-outline-secondary">View</a></td>
            </tr>
          <?php endforeach; ?>
          <?php if(empty($rows)): ?><tr><td colspan="7" class="text-muted p-3">No certs.</td></tr><?php endif; ?>
        </tbody>
      </table>
    </div>
